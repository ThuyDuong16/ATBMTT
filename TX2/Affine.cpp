...
// Hàm tính ngh?ch d?o modulo c?a a v?i n
int modInverse(int a, int n) {
	if(gcd(a, n) != 1)
		return 0;
	else {
...
int main() {
    string msg;
    cout << "Nh?p thông di?p: ";
    getline(cin, msg);

    cout << "Nh?p khóa mã hóa: " << endl;
    int a, b;
    do { 
        cout << "a = "; 
        cin >> a;
    } while(gcd(a, 26) != 1); // Ð?m b?o a và 26 là nguyên t? cùng nhau

    cout << "b = "; 
    cin >> b;

    cout << "Thông di?p g?c: " << msg << endl;

    string cipherText = encryptMessage(msg, a, b);
    cout << "Thông di?p mã hóa: " << cipherText << endl;  
    cout << "Thông di?p gi?i mã: " << decryptCipher(cipherText, a, b) << endl;
